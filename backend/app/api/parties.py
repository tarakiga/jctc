...
async def create_party(
    ...
    # Create party record (id and created_at auto-generated by BaseModel)
    db_party = Party(
    ...
    try:
        ...
        # Log PARTY_ADDED action for timeline
        from app.models.task import ActionLog
        action_log = ActionLog(
            case_id=party.case_id,
            user_id=current_user.id,
            action="PARTY_ADDED",
            details=f"{party.party_type.value} '{party.full_name or 'Unknown'}' added to case"
        )
        db.add(action_log)
        await db.commit()
...
async def update_party(
    ...
    try:
        ...
        # Log PARTY_UPDATED action for timeline
        from app.models.task import ActionLog
        action_log = ActionLog(
            case_id=db_party.case_id,
            user_id=current_user.id,
            action="PARTY_UPDATED",
            details=f"{db_party.party_type.value} '{db_party.full_name or 'Unknown'}' updated"
        )
        db.add(action_log)
        await db.commit()
...
async def delete_party(
    ...
    # Save party info before deletion for the log
    party_type = db_party.party_type.value
    party_name = db_party.full_name or 'Unknown'
    case_id = db_party.case_id
    ...
    try:
        ...
        # Log PARTY_REMOVED action for timeline
        from app.models.task import ActionLog
        action_log = ActionLog(
            case_id=case_id,
            user_id=current_user.id,
            action="PARTY_REMOVED",
            details=f"{party_type} '{party_name}' removed from case"
        )
        db.add(action_log)
        await db.commit()
